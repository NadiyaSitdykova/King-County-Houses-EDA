...
kc_house$formatted_date <- format(formatted_date, "%y %b")
kc_house <- transform(kc_house, price_thousands = price / 1000)
kc_house <- transform(kc_house, yr_build_or_renovated = max(yr_built, 
                                                            yr_renovated))

# process factors
kc_house$waterfront <- factor(kc_house$waterfront, levels = c(0, 1), 
                              ordered = T)
kc_house$view <- factor(kc_house$view, levels = c(0:4), 
                        ordered = T)
kc_house$condition <- factor(kc_house$condition, levels = c(1:5), 
                             ordered = T)
kc_house$day <-factor(kc_house$day, levels = c(1:31), 
                      ordered = T)
kc_house$month <- factor(kc_house$month, levels = c(1:12), 
                         ordered = T)
kc_house$formatted_date <- factor(kc_house$formatted_date, 
                                  levels = c("14 May", "14 Jun", "14 Jul", 
                                             "14 Aug", "14 Sep", "14 Oct", 
...
ggplot(data = kc_house, aes(x = formatted_date)) +
  geom_bar()
```
...
set.seed(20022012)
house_samp <- kc_house[sample(1:length(kc_house$price), 10000), 
                       c("price", "bedrooms", "bathrooms", "sqft_living", 
                         "sqft_lot", "yr_built")]
ggpairs(house_samp, 
...
ggplot(data = kc_house, 
       aes(x = round(sqft_living / 100) * 100, y = price_thousands)) +
  ...
  geom_line(stat = "summary", fun.y = "quantile", 
            fun.args = list(probs = .1), color = "blue", linetype = 2) +  
  geom_line(stat = "summary", fun.y = "quantile", 
            fun.args = list(probs = .9), color = "blue", linetype = 2)
```
...
with(kc_house, cor.test(price_thousands, as.numeric(waterfront), 
...
with(kc_house, cor.test(price_thousands, as.numeric(view), 
                        method = 'spearman', conf.level = 0.95, exact = FALSE))

```

```{r echo=FALSE, Bivariate_Plots_20_1}
view_anova <- with(kc_house, aov(price_thousands ~ view))
summary(view_anova)
```
```{r echo=FALSE, Bivariate_Plots_20_2}
with(kc_house, pairwise.t.test(price_thousands, view))
```
...
Variation of price have a tendency to increase with better levels of view. Spearmans rank shows us the a weak correlation. All pairs of view, except 1 and 2 appear to be significantly different at alpha = .05
...
with(kc_house, cor.test(price_thousands, as.numeric(condition), 
                        method = 'spearman', conf.level = 0.95, exact = FALSE))
```

```{r echo=FALSE, Bivariate_Plots_25_1}
condition_anova <- with(kc_house, aov(price_thousands ~ condition))
summary(condition_anova)
```

```{r echo=FALSE, Bivariate_Plots_25_2}
with(kc_house, pairwise.t.test(price_thousands, condition))
```
...
ggplot(data = kc_house, aes(x = formatted_date, y = sqft_living)) +
  geom_boxplot() +
  ylim(0, quantile(kc_house$sqft_living, 0.99))
```
...
with(kc_house, by(sqft_living, formatted_date, summary))
```
...
ggplot(data = kc_house, aes(x = formatted_date, y = price_thousands)) +
  geom_boxplot() +
  ylim(0, quantile(kc_house$price_thousands, 0.99))
```
...
with(kc_house, by(price_thousands, formatted_date, summary))
```

```{r echo=FALSE, Bivariate_Plots_32}
with(kc_house, cor.test(price_thousands, as.numeric(month), 
                        method = 'spearman', conf.level = 0.95, exact = FALSE))
```
...
```{r echo=FALSE, Multivariate_Plots_4_1}
view_anova <- with(kc_house, aov(price / sqft_living ~ view))
summary(view_anova)
```

```{r echo=FALSE, Multivariate_Plots_4_2}
with(kc_house, pairwise.t.test(price / sqft_living , view))
```
...
```{r echo=FALSE, Multivariate_Plots_6_1}
condition_anova <- with(kc_house, aov(price / sqft_living ~ condition))
summary(condition_anova)
```

```{r echo=FALSE, Multivariate_Plots_6_2}
with(kc_house, pairwise.t.test(price / sqft_living , condition))
```
...
ggplot(data = kc_house, aes(x = formatted_date, y = price / sqft_living)) + 
  geom_boxplot()
```
...
ggplot(data = kc_house, 
       aes(x = yr_built, y = price_thousands, color = condition)) +
...
ggplot(data = kc_house, 
       aes(x = yr_built, y = price_thousands, color = waterfront)) +
...
ggplot(data = kc_house_renovated, 
       aes(x = yr_renovated, y = price_thousands, color = condition)) +
  geom_point() +
  xlim(min(kc_house_renovated$yr_renovated), 
       quantile(kc_house_renovated$yr_renovated, 0.99)) +
...
ggplot(data = kc_house_renovated, 
       aes(x = yr_renovated, y = price_thousands, color = view)) +
  geom_point() +
  xlim(min(kc_house_renovated$yr_renovated), 
       quantile(kc_house_renovated$yr_renovated, 0.99)) +
  ylim(0, quantile(kc_house_renovated$price_thousands, 0.99)) +
...
ggplot(data = kc_house_renovated, 
       aes(x = yr_renovated, y = price_thousands, color = waterfront)) +
  geom_point() +
  xlim(min(kc_house_renovated$yr_renovated), 
       quantile(kc_house_renovated$yr_renovated, 0.99)) +
  ylim(0, quantile(kc_house_renovated$price_thousands, 0.99)) +
...
ggplot(data = kc_house, aes(x = round(sqft_living / 100) * 100, 
                            y = price_thousands, color = view)) +
...
ggplot(data = kc_house, aes(x = round(sqft_living / 100) * 100, 
                            y = price_thousands, color = condition)) +
  geom_point(alpha = 1/30) +
  geom_line(stat = "summary", fun.y = median) +
  xlim(0, quantile(kc_house$sqft_living, 0.99)) +
  ylim(0, quantile(kc_house$price_thousands, 0.99)) +
...
ggplot(data = kc_house, aes(x = round(sqft_living / 100) * 100, 
                            y = price_thousands, color = waterfront)) +
  geom_point(alpha = 1/30) +
  geom_line(stat = "summary", fun.y = median) +
  xlim(0, quantile(kc_house$sqft_living, 0.99)) +
  ylim(0, quantile(kc_house$price_thousands, 0.99)) 
...
m1 <- lm(formula = I(log10(price_thousands)) ~I((sqft_living)^(1/2)), data = kc_house)
mtable(m0, m1, sdigits = 3)
```

The model m0 has a little bit better R-squared, but let's see the residual plots for these two models.

```{r echo=FALSE, Multivariate_Plots_21}
kc_house$m0_res <- resid(m0)
kc_house$m0_fitted_value <- fitted(m0)
ggplot(data = kc_house, aes(x = m0_fitted_value, y = m0_res)) +
  geom_point(alpha = 1/20)
```

```{r echo=FALSE, Multivariate_Plots_22}
kc_house$m1_res <- resid(m1)
kc_house$m1_fitted_value <- fitted(m1)
ggplot(data = kc_house, aes(x = m1_fitted_value, y = m1_res)) +
  geom_point(alpha = 1/20)
```

From residual plot for m0 we can see that residuals contain predictive information. With higher fitted_value residuals tend to have larger absolute value. But for model m1 resuduals look more random. So we can say that model m1 has a better fit. Let's find out how
will fit the models with additional factors.
...
The variables in the linear model account for 51.8% of the variance in the price of houses. The addition of the view variable to the model improves the R^2 value by three percents, which is expected based on the visualization above of price vs sqft_living and view.
------
...
ggplot(data = kc_house, aes(x = formatted_date, y = price / sqft_living)) + 
  ...
  coord_cartesian(ylim = c(150, 350)) +
  ggtitle("Price per sqft by year and month") +
  theme(plot.title = element_text(hjust = 0.5)) +
  xlab("year and month") +
